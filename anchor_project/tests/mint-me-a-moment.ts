...
import { MintMeAMoment } from "../target/types/mint_me_a_moment";
import { assert } from "chai";

describe("mint-me-a-moment", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.MintMeAMoment as Program<MintMeAMoment>;
  ...
  it("fails when the tipper has insufficient balance", async () => {
